...
class UCFDetector(AbstractDetector):
    def __init__(self, config):
        ...
        # head
        specific_task_number = 5  # default: 5 in FF++
        self.head_spe = Head(
    ...
    def get_test_metrics(self):
        ...
        self.prob, self.label = [], []
        self.correct, self.total = 0, 0
        return {'acc':acc, 'auc':auc, 'eer':eer, 'ap':ap, 'f1':f1, 'precision':precision, 'recall':recall, 'mcc':mcc, 'pred':y_pred, 'label':y_true}

    def forward(self, data_dict: dict, inference=True) -> dict:
        ...
        if bs>2:
            ...
            if aug_idx < 0.7:
                ...
                print(f"f_share size: {f_share.size()}")
