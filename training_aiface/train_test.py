...
parser.add_argument('--train_batchsize', type=int, default=32, help="batch size")
...
parser.add_argument("--model", type=str, default='ucf',
...
parser.add_argument("--inter_attribute", type=str,
                    default='male,asian-male,white-male,black-male,others-nonmale,asian-nonmale,white-nonmale,black-nonmale,others-young-middle-senior-ageothers-hairothers-gray-blackh-blond-noeyewear-eyewear-otherseye')
...
parser.add_argument("--test_datapath", type=str,
                        default='./dataset/test_final.csv', help="test data path")
...
if args.dataset_type == 'pair':
    print("Fake dataset path:", args.datapath + 'fake_final.csv')
    print("Real dataset path:", args.datapath + 'real_final.csv')
    train_dataset = pairDataset(args.datapath + 'fake_final.csv', args.datapath + 'real_final.csv', data_transforms['train'])
    train_dataloader = DataLoader(dataset=train_dataset, batch_size=args.train_batchsize, shuffle=True, num_workers=8, pin_memory=True, collate_fn=train_dataset.collate_fn)
    train_dataset_size = len(train_dataset)
    
else:
    train_dataset = ImageDataset_Train(args.datapath + 'train_new.csv', data_transforms['train'])

    train_dataloader = DataLoader(dataset=train_dataset, batch_size=args.train_batchsize, shuffle=True, num_workers=8, pin_memory=True)
...
def train(model,  optimizer, scheduler, num_epochs, start_epoch):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, data_dict in enumerate(tqdm(train_dataloader)):
            ...
            imgs, labels, intersec_labels, age_labels, hair_labels, eye_labels= data_dict['image'], data_dict[
                'label'], data_dict['intersec_label'], data_dict['age_label'], data_dict['hair_label'], data_dict['eye_label']
            if 'label_spe' in data_dict:
                label_spe = data_dict['label_spe']
                data_dict['label_spe'] = label_spe.to(device)
            data_dict['image'], data_dict['label'], data_dict['intersec_label'], data_dict['age_label'], data_dict['hair_label'], data_dict['eye_label'] = imgs.to(
                device), labels.to(device), intersec_labels.to(device), age_labels.to(device), hair_labels.to(device) , eye_labels.to(device)
        ...
        if (epoch+1) % 1 == 0:
            ...
            for eachatt in interattributes:
                ...
                test_dataloader = DataLoader(
                    test_dataset, batch_size=args.test_batchsize, shuffle=False,num_workers=8, pin_memory=True)
            ...
            acc_fairness(args.savepath + '/', [['male', 'nonmale'], ['asian', 'white', 'black', 'others'],['young', 'middle','senior','ageothers'],['hairothers','gray','blackh','blond'],['noeyewear','eyewear','otherseye']])
            if epoch !=10:
...
def main():
    ...
    model, epoch = train(model, optimizer,
                         exp_lr_scheduler, num_epochs=11, start_epoch=start_epoch)
