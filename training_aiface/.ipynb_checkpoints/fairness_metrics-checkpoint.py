...
def acc_fairness(path, attribute_list):
    ...
    dict_data['group_5'] = {}
    dict_data['group_5']['label'] = []
    dict_data['group_5']['logits'] = []
    dict_data['group_5']['prediction'] = []
    ...
    if length == 5:
        ...
        for g4att in attribute_list[3]:
            if g4att not in dict_data.keys():
                dict_data[g4att] = {}
                dict_data[g4att]['label'] = []
                dict_data[g4att]['logits'] = []
                dict_data[g4att]['prediction'] = []

                label_path = path+g4att+"labels.npy"
                print(label_path, '')
                prediction_path = path+g4att+"predictions.npy"
                label_path = os.path.join(label_path)
                prediction_path = os.path.join(prediction_path)
                label = np.load(label_path)
                ...
                if len(label) != 0:
                    dict_data[g4att] = {}
                    dict_data[g4att]['label'] = label
                    dict_data[g4att]['logits'] = prediction
                    dict_data[g4att]['prediction'] = sigmoid(prediction)                  
        ...
        for g5att in attribute_list[4]:
            if g5att not in dict_data.keys():
                dict_data[g5att] = {}
                dict_data[g5att]['label'] = []
                dict_data[g5att]['logits'] = []
                dict_data[g5att]['prediction'] = []

                label_path = path+g5att+"labels.npy"
                print(label_path, '')
                prediction_path = path+g5att+"predictions.npy"
                label_path = os.path.join(label_path)
                prediction_path = os.path.join(prediction_path)
                label = np.load(label_path)
                ...
                if len(label) != 0:
                    dict_data[g5att] = {}
                    dict_data[g5att]['label'] = label
                    dict_data[g5att]['logits'] = prediction
                    dict_data[g5att]['prediction'] = sigmoid(prediction)                  
                    dict_data['group_5']['label'] += (list(label))
                    dict_data['group_5']['logits'] += (list(prediction))
                    dict_data['group_5']['prediction'] += (
                        list(sigmoid(prediction)))
    ...
    gap_g1, gap_g2, gap_g3,  gap_inter, gap_g4, gap_g5= {}, {}, {},{}, {}, {}
    ...
    gap_g4['auc'], gap_g4['ErrorRate'], gap_g4['FPR'], gap_g4['acc'], gap_g4['foae'], gap_g4['PR'], gap_g4['NR'], gap_g4['Fdp_diff'], gap_g4['F_DP'], gap_g4['FNR'], gap_g4['TNR'], gap_g4['TPR'] = [
    ], [], [], [], [], [], [], [], [], [], [], []
    gap_g5['auc'], gap_g5['ErrorRate'], gap_g5['FPR'], gap_g5['acc'], gap_g5['foae'], gap_g5['PR'], gap_g5['NR'], gap_g5['Fdp_diff'], gap_g5['F_DP'], gap_g5['FNR'], gap_g5['TNR'], gap_g5['TPR'] = [
    ], [], [], [], [], [], [], [], [], [], [], []
    ...
    gap_g1['Fmeo_diff'], gap_g2['Fmeo_diff'], gap_g3['Fmeo_diff'], gap_g4['Fmeo_diff'], gap_g5['Fmeo_diff'], gap_inter['Fmeo_diff'] = [], [], [], [], [],[]
    for index, group in enumerate(attribute_list[:4]):
        a = 0.0
        b = 0.0
        efpr = 0.0
        cov = 0.0
    ...
    for g4 in attribute_list[3]:
        try:
            gap_g4['auc'].append(metrics[g4]['auc'])
            gap_g4['ErrorRate'].append(metrics[g4]['ErrorRate'])
            gap_g4['FPR'].append(metrics[g4]['FPR'])
            gap_g4['acc'].append(metrics[g4]['acc'])
            gap_g4['foae'].append(metrics[g4]['acc'])
            gap_g4['PR'].append(metrics[g4]['positive_rate'])
            gap_g4['NR'].append(metrics[g4]['negative_rate'])
            gap_g4['FNR'].append(metrics[g4]['FNR'])
            gap_g4['TNR'].append(metrics[g4]['TNR'])
            gap_g4['TPR'].append(metrics[g4]['TPR'])
            
        except:
            continue

    gap_g4['Fdp_diff'].append(max(gap_g4['PR'])-min(gap_g4['PR']))
    gap_g4['Fdp_diff'].append(max(gap_g4['NR'])-min(gap_g4['NR']))
    gap_g4['Fmeo_diff'].append(max(gap_g4['FPR'])-min(gap_g4['FPR']))
    gap_g4['Fmeo_diff'].append(max(gap_g4['FNR'])-min(gap_g4['FNR']))
    gap_g4['Fmeo_diff'].append(max(gap_g4['TNR'])-min(gap_g4['TNR']))
    gap_g4['Fmeo_diff'].append(max(gap_g4['TPR'])-min(gap_g4['TPR']))

    for g5 in attribute_list[4]:
        try:
            gap_g5['auc'].append(metrics[g5]['auc'])
            gap_g5['ErrorRate'].append(metrics[g5]['ErrorRate'])
            gap_g5['FPR'].append(metrics[g5]['FPR'])
            gap_g5['acc'].append(metrics[g5]['acc'])
            gap_g5['foae'].append(metrics[g5]['acc'])
            gap_g5['PR'].append(metrics[g5]['positive_rate'])
            gap_g5['NR'].append(metrics[g5]['negative_rate'])
            gap_g5['FNR'].append(metrics[g5]['FNR'])
            gap_g5['TNR'].append(metrics[g5]['TNR'])
            gap_g5['TPR'].append(metrics[g5]['TPR'])
            
        except:
            continue

    gap_g5['Fdp_diff'].append(max(gap_g5['PR']) - min(gap_g5['PR']))
    gap_g5['Fdp_diff'].append(max(gap_g5['NR']) - min(gap_g5['NR']))
    gap_g5['Fmeo_diff'].append(max(gap_g5['FPR']) - min(gap_g5['FPR']))
    gap_g5['Fmeo_diff'].append(max(gap_g5['FNR']) - min(gap_g5['FNR']))
    gap_g5['Fmeo_diff'].append(max(gap_g5['TNR']) - min(gap_g5['TNR']))
    gap_g5['Fmeo_diff'].append(max(gap_g5['TPR']) - min(gap_g5['TPR']))
    ...
    for i in ['FPR', 'auc', 'ErrorRate', 'foae']:
        try:
            print(
                f' inter_{i}, g1_{i}, g2_{i}, g3_{i}, g4{i}, g5{i}: {(100*(max(gap_inter[i])-min(gap_inter[i]))):.3f} | {(100*(max(gap_g1[i])-min(gap_g1[i]))):.3f} | {(100*(max(gap_g2[i])-min(gap_g2[i]))):.3f} | {(100*(max(gap_g3[i])-min(gap_g3[i]))):.3f} | {(100*(max(gap_g4[i])-min(gap_g4[i]))):.3f} | {(100*(max(gap_g5[i])-min(gap_g5[i]))):.3f}')
            return_results += [(100*(max(gap_inter[i])-min(gap_inter[i]))), (100*(
                max(gap_g1[i])-min(gap_g1[i]))), (100*(max(gap_g2[i])-min(gap_g2[i]))), (100*(max(gap_g3[i])-min(gap_g3[i]))), (100*(max(gap_g4[i])-min(gap_g4[i]))), (100*(max(gap_g5[i])-min(gap_g5[i])))]
        except:
            continue
         
    print(
        f' inter_F_DP,g1_F_DP,g2_F_DP,g3_F_DP, g4_F_DP, g5_F_DP:{100*max(gap_inter["Fdp_diff"]):.3f} | {100*max(gap_g1["Fdp_diff"]):.3f} | {100*max(gap_g2["Fdp_diff"]):.3f} | {100*max(gap_g3["Fdp_diff"]):.3f} | {100*max(gap_g4["Fdp_diff"]):.3f} | {100*max(gap_g5["Fdp_diff"]):.3f}')
    print(
        f' inter_F_meo,g1_F_meo,g2_F_meo, g3_F_meo, g4_F_meo ,g5_F_meo:{100*max(gap_inter["Fmeo_diff"]):.3f} | {100*max(gap_g1["Fmeo_diff"]):.3f} | {100*max(gap_g2["Fmeo_diff"]):.3f} | {100*max(gap_g3["Fmeo_diff"]):.3f} | {100*max(gap_g4["Fmeo_diff"]):.3f} | {100*max(gap_g5["Fmeo_diff"]):.3f}')
